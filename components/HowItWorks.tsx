...
const Module = ({ protocol, index, total }: { protocol: typeof PROTOCOLS[0], index: number, total: number }) => {
  ...
  return (
    <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24 mb-40 last:mb-0 items-center">
      {/* Content Side */}
      <motion.div 
      ...
      >
        ...
        <h3 className="text-4xl md:text-6xl font-black text-indigo-950 mb-4 tracking-tighter">
          {protocol.title}
        </h3>
        <p className="text-indigo-600 font-bold uppercase tracking-[0.2em] text-xs mb-8">
          {protocol.tagline}
        </p>
        <p className="text-indigo-900/60 text-lg md:text-xl font-medium leading-relaxed max-w-lg">
      ...
      >
        ...
        <div className="relative bg-white border border-indigo-50 shadow-[0_32px_64px_-16px_rgba(49,46,129,0.1)] rounded-[3rem] p-10 md:p-16 overflow-hidden">
          {/* Animated decorative corner */}
          <div className="absolute top-0 right-0 w-32 h-32 bg-indigo-50/50 rounded-bl-[4rem] -translate-y-8 translate-x-8"></div>
          
          <div className="w-full aspect-square text-indigo-900/20 group-hover:text-indigo-600 transition-colors duration-500">
            {protocol.icon}
          </div>
          
          {/* Module metadata overlay */}
          <div className="absolute bottom-8 right-8 text-[8px] font-mono text-indigo-300 uppercase tracking-widest opacity-40">
...
const HowItWorks = () => {
  ...
  return (
    <section id="how-it-works" className="py-40 bg-slate-50 relative overflow-hidden font-sans">
      ...
      <div className="max-w-7xl mx-auto px-6 relative">
        ...
        <div className="max-w-3xl mb-40">
          <motion.div 
            ...
            viewport={{ once: true }}
            className="inline-flex items-center gap-3 bg-white border border-indigo-100 px-5 py-2 rounded-full mb-8 shadow-sm"
          >
            <span className="flex h-2 w-2 rounded-full bg-indigo-600">
              <span className="animate-ping absolute inline-flex h-2 w-2 rounded-full bg-indigo-400 opacity-75"></span>
            </span>
            <span className="text-indigo-600 font-black text-[10px] uppercase tracking-[0.3em]">Engine Architecture</span>
          </motion.div>
          <motion.h2 
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            className="text-6xl md:text-8xl font-black text-indigo-950 tracking-tighter leading-[0.9] mb-10"
          >
          ...
          <motion.p 
            ...
            className="text-xl md:text-2xl text-indigo-900/60 font-medium leading-relaxed"
          >
